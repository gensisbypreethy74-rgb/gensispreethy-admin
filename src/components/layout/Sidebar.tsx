...
const navItems = [
  ...
  },
  {
    ...
    icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
  },
  {
    name: 'Customers',
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex w-[260px] bg-[#0A0A0A] text-slate-300 flex-col shrink-0">
        <div className="flex items-center gap-4 h-[100px] px-8 shrink-0 sticky top-0 bg-[#0A0A0A] z-10 border-b border-slate-800">
      ...
      {/* Drawer */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-[260px] bg-[#0A0A0A] text-slate-300 flex flex-col transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="flex items-center justify-between h-[80px] px-6 shrink-0 sticky top-0 bg-[#0A0A0A] z-10 border-b border-slate-800">
          <div className="flex items-center gap-4">
